...
This document discusses why `@seneca/salesforce-provider` is built the way it is.
...
The payoff is uniformity. An application reading from Salesforce Account sObject, a
...
`provider/salesforce/` canon, registering the `role:entity` messages
...
*already* entity-shaped. The Salesforce Account sObject SDK exposes accessors like
`client.Account()` — carrying
...
`account`, and the store surface those
...
`salesforce` and sends the `apikey` as a bearer token when one is
configured.
...
The SDK is an ordinary published dependency: `@voxgig-sdk/salesforce` at
...
[@voxgig/sdkgen](https://github.com/voxgig/sdkgen) from the Salesforce Account sObject API
...
The source of truth is the SDK project's model — `../../voxgig-sdk/salesforce-sdk` from
...
.use('@seneca/salesforce-provider', {
  test: true,
  testopts: { entity: { account: { 'account0': { ... } } } },
})
```

This is better than the usual provider-testing compromise. Rather than checking
only that the plugin loads and answers
`sys:provider,provider:salesforce,get:info`, the tests exercise the
entity commands themselves — list, load, the not-found answer —
